...
2. **状态栏**:窗口内容区顶部一行,左侧实时显示 harness 运行状态(运行中/已停止 + 端口),右侧两个按钮。
3. **两个弹框**:
...
| 决策项 | 结论 |
|---|---|
| 状态栏形态 | 保留系统标题栏(最小化/最大化/关闭不动),状态栏是标题栏下方的独立一行 |
...
**状态栏挂载**:webview 是 GtkWindow 的直接子控件(`gtk_bin_get_child`)。用 `gtk_container_remove` 摘下,放入新建的纵向 GtkBox,顶部插一行横向 GtkBox:
