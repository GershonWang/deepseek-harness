...
5. **宿主条目按菜单读取重解析。** 文档原为「每插件生命周期解析一次」(§4 的每趟一次探测)。实际行为是每次 apps 路由读取都就地把宿主逃逸条目重解析一遍并按目录顺序写回,而沙箱内解析出的条目保留解析它的那一趟:沙箱宿主上安装的应用会随进程运行而变化,而宿主是唯一会在进程之下变化的机器。该重解析只在声明了通道时进行,因此沙箱之外的检测保持每进程一次的开销;浏览器侧在打开菜单时索取最新列表。
