...
5. **Host entries refresh per menu read.** The document resolved the catalog once per plugin life (section 4's one probe per pass). The shipped behavior re-resolves the host-escape entries on every apps-route read, in place and in catalog order, while entries resolved inside the sandbox keep their pass: a sandbox host's installed applications change while the process lives, and that host is the only machine that changes under it. The refresh runs only under a declared channel, so detection outside a sandbox keeps its once-per-process cost, and the browser half asks for the fresh list as the menu opens.
