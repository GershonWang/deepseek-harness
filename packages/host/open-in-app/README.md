...
- **Inside a sandbox** that declares a host-escape channel ([launch-environment](../../util/launch-environment/README.md)), Linux entries also read the host's own desktop entries: only the ids each entry documents, from the host's `/usr/local/share`, `/usr/share`, the Linglong store's exported entries, and the home shared with the host. The entry's `Exec` program must exist on the host, and the launch runs that host path through the sandbox's forwarding launcher on the host user manager. One probe decides whether the channel is offered at all, so a sandbox whose launcher cannot start a host process lists no host applications instead of entries that fail on click, and a menu read re-resolves these host entries — a program installed or removed on the host reaches the next menu without restarting anything; a host outside a sandbox resolves exactly as before.
...
Resolution runs lazily on the first request that needs it, and each menu read re-resolves the sandbox's host-escape entries in place, so a host install or removal reaches the menu as it opens; detection inside the sandbox, and every host without a declared channel, stays one pass per process, where installing an application takes effect on the next restart. An uninstalled one heals immediately either way — a launch that finds its executable gone re-resolves that one entry and drops it from the list when nothing proves it anymore. The icon route serves the real application icon on every platform where one is extractable: the bundle's `.icns` as a 128px PNG on macOS, the executable's associated icon as a 32px PNG on Windows, and the desktop entry's hicolor-theme icon (PNG or SVG) on Linux; a missing icon answers 404 and the browser surface renders a generic glyph.
...
- **Icon fidelity is platform-bound.** Windows icons come from `ExtractAssociatedIcon` at 32px — the most the stock .NET surface yields without a native addon — which can render slightly soft on high-DPI displays; Linux icons follow the hicolor theme and pixmaps only, not the user's active icon theme; several entries (CLI-only launchers without a desktop entry) have no icon source and keep the generic glyph; the apps route lists those ids in `iconless` so the browser never requests their icon.
- **The menu follows the host, not the sandbox.** A menu read re-resolves a sandbox's host-escape entries, so a program installed or removed on the host reaches the next menu; an application installed inside the sandbox, or on any host without a declared channel, still appears only after a restart, where only the uninstall direction self-heals (a missing launcher re-resolves its one entry on the spot).
- **Host applications come from a declared, verified channel.** Inside a sandbox the host view is exactly the directories above plus the home shared with the host; a per-user `~/.local/share/applications` is writable by that user, so an entry there is trusted only as far as the user's own home is. The channel is offered while the sandbox declares it and one probe starts a host process; a host user manager that becomes unreachable mid-session is re-checked by the next stale-launch refresh and withheld once the probe fails.
