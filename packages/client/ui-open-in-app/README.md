...
The Session header and document header use the same 24px-high split button with 9px corners. Both headers show only the icon; their tooltip names the default application or the reveal action. Both show the default action’s icon, mark its application with “(default)” in the menu, disable while their own gesture runs, and report failures through a transient toast. The directory adapter uses the existing cross-platform application catalog, read once per page and again as its menu opens so the list follows the host, and remembers the last successful choice in `dsh.open-in-app.choice`; a missing choice falls back to the first available application.
...
- **Availability follows the host's policy for the directory catalog.** The client asks the host again whenever that menu opens, so a sandbox host-escape channel picks up host installs on the next open, while an application installed inside the sandbox, or on a host without a channel, appears after a reload and a host restart. The desktop answer behind the file controls is read once per page as well.
