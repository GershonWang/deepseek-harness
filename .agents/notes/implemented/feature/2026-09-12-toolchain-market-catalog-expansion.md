...
The market grid's tracks are `minmax(0, 1fr)`, not `1fr`. The two are not the same: `1fr` is `minmax(auto, 1fr)`, so a track's minimum came from the min-content of the cards in it, and a card's min-content is set by its meta row — a `white-space: nowrap` command list. Cards with long lists (sqlite's four commands, pandoc's three, and Rust's five before them) pushed their tracks past an equal share; once the tracks summed past the grid width, the grid overflowed horizontally and the right column was clipped by the dialog edge. With the floor at zero the tracks divide equally and the meta row ellipsizes through its own `overflow: hidden` and `text-overflow: ellipsis`. The pre-existing `1fr` had already made the three tracks unequal (262/320/265 at the time), so the expansion did not introduce the mechanism — it added the first cards long enough to push the total past the container.
