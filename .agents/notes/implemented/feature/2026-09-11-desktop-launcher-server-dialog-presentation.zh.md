...
连接模式行、两张面板、错误行在服务器弹框正文里各占固定的网格行（`.modal-body-stack`），两张面板共用第 2 行。卡片高度因此在任何时候都取较高那张面板的高度，切换连接模式不再改变弹框大小：远端面板只有一个输入行，容器内面板约 183 px，原先的跳变约 125 px。

叠放成员在不活跃时仍然贡献高度。在叠放容器内，`.hidden` 不再等于 `display: none`，而是被覆盖为 `visibility: hidden`：元素留在流内，但不可点击、不可聚焦、也不进入无障碍树。地址的两种呈现用同样的方式叠放，值列因此始终按地址框量高，而不是按当前显示的那一种；运行中 → 启动中 → 已停止 之间的切换也不再让卡片移动约 25 px。

地址行按居中对齐（`.row-address`），因为当网格项都不参与基线对齐时，网格容器会由边框盒合成基线，那会把「地址」标签拽到地址框底部。

`.addr` 预留两行高度（`min-height: calc(2.9em + 8px)`），因此即使本次会话从未运行过 harness、框内还是空的，这一行也保持同样高度。

仍有两处高度变化是刻意的：失败态的安全模式块，以及安全模式与全新环境徽标，依然会让弹框变高。它们是在已有内容之外新增操作，而不是替换已有内容。

远端面板改成按容器内模式的信息组织，好让它自己的内容就长到那个高度、而不是留出一片空场：服务地址是多行输入框，从三行起随剩余高度生长、最多六行封顶；连接与断开落在右下，共用容器内面板那条 `.actions.actions-bar` 分隔线。已连接提示写「已连接 <host>:<port>」而不是重复完整地址——上面那个输入框里已经有它，状态栏也已收窄为同一标签——并给这一行预留一行高度，因此连接成功不再给卡片添一行。
...
**给卡片一个 `min-height` 或 `height`，而不是做叠放。** 一行声明、不动结构。否决：那个值必须按最高的状态来调，于是其余每个状态都常年留白，而且一旦增删一行、换字体或换主题就对不上了。叠放量的是内容本身。

**给远端面板定高，或把它那一行垂直居中。** 否决：输入框仍会把长地址的尾巴推进横向滚动里，而把一行内容在一张 170 px 的卡片里居中，只是把那片空场挪个位置，并没有填掉它。

**保留完整地址的已连接提示，并为此预留三行。** 否决：预留三行的代价与打印三行相同，而且会把输入框上方已经显示过的持有者凭据再重复一遍。

**继续用 `display: none`，改为往远端面板里填一段引导页的说明文案。** 可以把空白用起来。否决：它必须与容器内面板高度分毫不差才有效果，于是又回到那个调出来的数字，还要额外维护一段文案。

**在单个值节点里插入换行并设 `white-space: pre-line`。** 比拆分元素少：一个节点、一次 `textContent` 写入。否决：一个文本节点只有一种颜色与一种字重，位置与凭据会读起来同等重要，而弱化的令牌行恢复的正是这个层级。

## Consequences

弹框现在读起来与工具链弹框同属一个壳，地址行的布局也不再取决于 token 里是否含断行字符——常见情况下它一个都没有，这正是过去断点会落在令牌中间的原因。地址占两行而不是一行，复制按钮相对这两行垂直居中。远端模式下卡片保持容器内面板的高度，而地址输入框会长进那片高度里，因此空间是被填掉而不是留白；输入框同时封顶六行，免得异常高的叠放把它无限撑大。状态栏不再提供一个可以选中并粘进浏览器的值；这条路径改为弹框的复制按钮，它交出的是 Web 服务真正接受的那个 URL。`hostLabel` 遇到无法解析的地址时返回空标签而不是回退到原串，因此畸形地址绕不开脱敏——代价是畸形地址在状态栏上不显示任何位置。

## Testing

`node --test frontend/test-app.cjs` 跑 37 例，其中五例覆盖本次改动：运行态与启动态下状态行的语义类；运行态显示地址元素并隐藏文案元素、进入启动中后反过来；主机端口段与令牌段各自落在自己的元素里；地址没有查询串时令牌行为空；容器模式与外部模式下状态栏文本包含主机端口且不含 token。测试构建的 vm 沙箱现在会拿到 `URL`——缺了它 `hostLabel` 会走解析失败的兜底分支，脱敏断言就会因为错误的原因通过。布局本身没有用例：DOM 桩既不实现样式表也不实现布局。

布局改由 DevTools 协议在无头 Chromium 里实测：页面是去掉脚本后逐字复制的 `index.html`，每个状态按 `app.js` 的写法还原。卡片高度（CSS px，深浅主题一致）：容器内运行中 308、容器内启动中 307、外部空 307、外部连接中 307、外部已连接 307。因此切换模式不动分毫，切换状态最多只有 1 px——那是 PID 行空行盒与实行盒取整不同造成的。刻意的例外是失败态带安全模式块时，398 px。

同一套工具抓出了 DOM 桩抓不到的两个缺陷：已连接提示打了三行完整地址、把卡片撑到 339 px；异常高的叠放把地址输入框撑到 174 px。两者均已在上面修掉，这里列的是修复后的实测值。
