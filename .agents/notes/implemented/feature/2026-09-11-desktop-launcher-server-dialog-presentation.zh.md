...
弹框现在读起来与工具链弹框同属一个壳，地址行的布局也不再取决于 token 里是否含断行字符——常见情况下它一个都没有，这正是过去断点会落在令牌中间的原因。地址占两行而不是一行，复制按钮相对这两行垂直居中。远端模式下卡片保持容器内面板的高度，于是单独一个输入行上方会留出约 125 px 空白；这是"高度不再移动"的代价，而那处空间也不会被容器内面板用到的内容占用。状态栏不再提供一个可以选中并粘进浏览器的值；这条路径改为弹框的复制按钮，它交出的是 Web 服务真正接受的那个 URL。`hostLabel` 遇到无法解析的地址时返回空标签而不是回退到原串，因此畸形地址绕不开脱敏——代价是畸形地址在状态栏上不显示任何位置。

## Testing

`node --test frontend/test-app.cjs` 跑 24 例，其中五例覆盖本次改动：运行态与启动态下状态行的语义类；运行态显示地址元素并隐藏文案元素、进入启动中后反过来；主机端口段与令牌段各自落在自己的元素里；地址没有查询串时令牌行为空；容器模式与外部模式下状态栏文本包含主机端口且不含 token。测试构建的 vm 沙箱现在会拿到 `URL`——缺了它 `hostLabel` 会走解析失败的兜底分支，脱敏断言就会因为错误的原因通过。

定高没有测试：它是 CSS 布局，而这套测试构建的 DOM 桩既不实现样式表也不实现布局。它需要与其余呈现部分同样的目视核对。
