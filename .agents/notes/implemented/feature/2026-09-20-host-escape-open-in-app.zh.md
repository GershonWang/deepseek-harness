...
宿主条目是解析结果中唯一会随进程存续而变化的部分，因此 apps 路由每次读取都会重解析它们——一次探测、宿主数据目录、逐条校验 `Exec` 程序——并按目录顺序写回同一个映射，而沙箱内解析出的条目保留解析它的那一趟。该重解析只在声明了通道时发生：沙箱之外、以及沙箱自身的检测，解析仍是历来的一次/进程；图标路由挂到最近一次菜单读取发布的结果上，而不是每个图标都重解析一次。
