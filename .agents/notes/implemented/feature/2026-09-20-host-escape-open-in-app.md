...
The host entries are the one part of a resolved catalog that moves while the process lives, so every apps-route read re-resolves them — one probe, the host's data directories, each candidate's `Exec` program — and rewrites them into the map in catalog order, while entries resolved inside the sandbox keep the pass that resolved them. The refresh runs only under a declared channel: outside a sandbox, and for the sandbox's own detection, resolution stays the once-per-process pass it has always been, and the icon route attaches to the values the last menu read published rather than re-resolving once per icon.
