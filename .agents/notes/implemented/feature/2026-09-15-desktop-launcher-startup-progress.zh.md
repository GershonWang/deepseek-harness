...
这个计数可信有一个值得记录的原因：分母取 `loader.entries()` 里真正会挂载的行（排除 group、排除 disabled），它在启动期是稳定的。分子是已 settle 的 fiber，因此只增不减；而上报在「第一次计数到齐」时停止——这条通道只覆盖启动窗口，就绪之后 harness 仍会继续重组配置树。

在已发布的客户端上实测（2026-09-15 16:07 那次启动，`~/.cache/dsh-desktop/harness.log`）：第一行落在 spawn 之后 1.24 秒、值 `0/133`，计数在 5.65 秒首次到齐为 `135/135`，就绪行在 5.77 秒跟上——也就是说加载页约 4.4 秒在走一个真实计数，`serving` 阶段只有 0.12 秒。开发态同一套上报器在 1.45 秒报 `0/127`、3.71 秒报 `129/129`（就绪 3.73 秒）；打包态在 1.15 秒报 `0/127`、3.27 秒报 `129/129`（就绪 3.3 秒）。
...
一次启动的前约 1.2 秒没有计数：上报器在被挂载之前无法上报，而在已发布的客户端上它的第一行正是 `0/133`。这段空窗正是 `loading` 阶段存在的理由——它是一条真实事实（"子进程已经在说话，计数还没开始"），不是填充物。

分母可能在到齐之前再多出一两行，因此比率可能回退一个百分点。页面把进度条按单调不减渲染，同时原样显示上报的数字，于是显示不会倒退，而计数保持精确。

上报必须在到齐时停止，而第一次真实运行就证明了这一点：上报器原先会继续计数，于是就绪态自身的重组（客户端 HMR、用户补丁层 watcher、目录选择器）把新行也算进分子，而分母取自当时活着的树，结果产生 26 行分子大于分母的日志——最后一条是 `161/136`。加载页并不受影响（就绪后它已被 iframe 取代，且前端把比例夹在 100%），但这些行污染了本仓库正是要读这份日志来取的启动耗时数据，也与该行自身的契约矛盾。在「第一次计数到齐」处停止，最后一次上报便诚实地停在 100%；而计数始终到不齐的异常启动仍会继续上报直到到齐。

只有桌面启动器拉起 harness 时才有这条进度通道。浏览器里由 `dsh web` 提供的页面仍然显示 harness 自己的客户端加载页——那是另一个界面，按决策不在范围内。

## Testing

`internal/supervisor` 锁定 stderr 契约（含畸形行与无关行的模式表）、经真实子进程验证的 writer 接线（`testdata/mock-progress.sh`，同时断言 `OutputAt` 落在 spawn 窗口内），以及共用的行切分 sink 跨写入边界的行为。`internal/appenv` 锁定 overlay 行、其 YAML 转义后的绝对路径、与市场补丁共存、插件的前缀契约，以及插件源码不含任何 import。`node --test internal/appenv/startup_progress.test.mjs` 用手写上下文驱动插件：播种与逐条 settle、group 与 disabled 不进分母、同一 entry id 只计一次、到齐后停止上报（就绪后新增的行不得产生任何输出）、激活被拒绝也算已激活、缺少 loader 时静默、上报器自身出错只写一行 `unavailable` 而不抛。`internal/app` 锁定阶段映射表（含 `0/0`，以及所有非启动态映射到零值视图）与让进度变化能到达前端的快照比较。
...
两种 harness 启动方式都用手工跑过真实 overlay 与上报器：开发态（`apps/cli/lib/bin.js`）与打包态（`linglong/output/binary/files/harness/lib/bin.js`，其条目名经 `HostResolvedRootInclude` 解析）都成功加载了注入的绝对路径，上报 `0/127` 到 `129/129`，且就绪行之后不再产生任何进度行。
