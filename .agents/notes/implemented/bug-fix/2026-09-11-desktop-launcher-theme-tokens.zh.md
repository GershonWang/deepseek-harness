...
**把深色的 `--brand` 调暗，让主按钮上的白字达到 4.5:1。** 该填充色是 `#4d6bfe`，只有 4.33:1。本次否决：它改的是品牌填充色本身，那属于产品识别决策而不是 token 化决策。此项目前仍开放（见 Consequences）。
...
仍有两处已知缺口，记录在此而不是就地修掉。深色主题主按钮上的白字实测 4.33:1，低于 4.5:1 的文字阈值；要消除它就得改品牌填充色，本次刻意没有做。深色主题关闭键 hover 实测 3.57:1，高于纯图标控件适用的 3:1 阈值。
