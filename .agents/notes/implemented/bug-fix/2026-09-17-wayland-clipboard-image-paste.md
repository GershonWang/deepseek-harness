...
The X11 session was then verified on the machine as well. After switching sessions, the running client reported `XDG_SESSION_TYPE=x11`, `DISPLAY=:0`, `XAUTHORITY=/run/linglong/Xauthority` and no `WAYLAND_DISPLAY`, with Xorg on `:0`, and manual acceptance passed the same four paths. Driving `ReadImage()` with that session's real environment read 25151 bytes from a `CLIPBOARD` bitmap, from a `CLIPBOARD` `text/uri-list`, and from a `PRIMARY` bitmap. The `PRIMARY` case returned 0 bytes once on the first attempt, before the selection owner was checked; three re-runs of the identical sequence, each confirming the owner held 25151 bytes first, all passed, so it was a harness race rather than a defect in the code — the mechanism was not captured.
