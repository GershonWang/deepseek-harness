...
`apps/desktop-launcher/frontend/tools/preview.mjs` renders `index.html` in headless Chromium and measures it. It is a plain Node ESM script with no dependencies: the browser comes from `DSH_PREVIEW_BROWSER`, otherwise the Playwright cache already on the machine, otherwise `PATH`, and the protocol runs over Node's built-in `WebSocket`. Chromium's profile and `HOME`/XDG directories are redirected into a directory created per run under `apps/desktop-launcher/.preview-cache`, without which it stalls against a read-only home under a file sandbox. That root sits outside `frontend/` because the launcher embeds `frontend/locales` and `frontend/vendor` as whole directories, so anything written there ships, while a stray file elsewhere under `frontend/` fails the embed-list assertion in `main_test.go`.
