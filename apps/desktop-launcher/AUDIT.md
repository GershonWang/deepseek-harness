...
- 仓库当前的文档闸门并非全绿。**2026-09-16 实跑**：`verify-translation-pairing` 报 33 处缺配对、**1 行 out-of-sync**、4 处 link target diverges；`verify-md-links` 报 7 行（`bundle-xdg-open` 与 `generic-file-attachments` 两对 Note 的链接目标不存在）；`verify-md-wrap` 报 `docs/superpowers/**` 下的硬换行；`verify-package-readme-limitations` 报 `packages/support/doctor/README.md` 缺 `## Known Limitations and Deferred Work` 小节。
- **上面那 1 行 out-of-sync 的来历**（`apps/desktop-launcher/README` 那一对原也在其中，已处理）：`apps/desktop-launcher/README.md`/`README.zh.md` 由本文自身造成——附录 D 的 S7 修复提交 `593e30a184` 同改了中英两侧，却没重录 `README.i18n.yaml`（记录值 `8e3350c0…`/`986eaaae…` 对当时的 `51f2c9f9…`/`9a3cb439…`）。已核实两侧改动一一对称（`.preview` 路径与 `all:frontend` 警告的措辞、范围同时改写）后重录该配对。**剩下这一行尚未处理**：`.agents/notes/implemented/feature/2026-08-14-desktop-launcher-linux-linglong` 这一对，`0cad0f40e4` 单独修正了中文侧语言切换的自指链接（`[中文]` 指向 `.zh.md`），英文侧无需对应改动，但同样没有重录记录。
