...
| [doctor-migration-plan.md](./doctor-migration-plan.md) | **doctor 怎么搬进启动器、每一步怎么做**——把自研诊断能力（doctor 包、`dsh doctor` 入口、`DSH_SAFE_MODE`）全部收敛进 `apps/desktop-launcher/` 的可执行方案：目标架构、影响面清单、阶段 0 spike、四阶段任务（含 TDD 步骤与确切命令）、验收标准、待决策项与回滚策略 | 1226 行 |
